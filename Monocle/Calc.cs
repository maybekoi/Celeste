...
namespace Monocle
{
    ...
    public static class Calc
    {
      ...
      {
        return Enum.IsDefined(typeof (T), (object) str) ? (T) Enum.Parse(typeof (T), str) : throw new Exception("The string cannot be converted to the enum type.");
      }
      ...
      public static Color HexToColor(string hex)
      {
        ...
        {
          double r = (double) ((int) Calc.HexToByte(hex[num1]) * 16 /*0x10*/ + (int) Calc.HexToByte(hex[num1 + 1])) / (double) byte.MaxValue;
          float num2 = (float) ((int) Calc.HexToByte(hex[num1 + 2]) * 16 /*0x10*/ + (int) Calc.HexToByte(hex[num1 + 3])) / (float) byte.MaxValue;
          float num3 = (float) ((int) Calc.HexToByte(hex[num1 + 4]) * 16 /*0x10*/ + (int) Calc.HexToByte(hex[num1 + 5])) / (float) byte.MaxValue;
      ...
      {
        return time.TotalHours >= 1.0 ? $"{(object) (int) time.TotalHours}:{time.ToString("mm\\:ss\\.fff")}" : time.ToString("m\\:ss\\.fff");
      }
      ...
      {
        return (double) Math.Abs(value) >= (double) threshold ? (float) Math.Sign(value) : 0.0f;
      }
      ...
      {
        return negative ? (positive ? both : -1) : (positive ? 1 : 0);
      }
      ...
      {
        return (double) value <= 0.5 ? value * 2f : (float) (1.0 - ((double) value - 0.5) * 2.0);
      }
      ...
      {
        return (float) (((double) angleRadians * (double) Math.Sign(angleRadians) + 3.1415927410125732) % 6.2831854820251465 - 3.1415927410125732) * (float) Math.Sign(angleRadians);
      }
      ...
      {
        return (double) val <= (double) target ? Math.Min(val + maxMove, target) : Math.Max(val - maxMove, target);
      }
      ...
      {
        return (double) Math.Abs(Calc.AngleDiff(currentAngle, angleA)) < (double) Math.Abs(Calc.AngleDiff(currentAngle, angleB)) ? angleA : angleB;
      }
      ...
      {
        return from == to ? Vector2.Zero : (to - from).SafeNormalize(length);
      }
      ...
      {
        return (double) amount > (double) (target - v).Length() ? target : v + (target - v).SafeNormalize() * amount;
      }
      ...
      {
        return !xml.HasAttr(attributeName) ? defaultValue : Convert.ToInt32(xml.Attributes[attributeName].InnerText);
      }
      ...
      {
        return Enum.IsDefined(typeof (T), (object) xml.InnerText) ? (T) Enum.Parse(typeof (T), xml.InnerText) : throw new Exception("The attribute value cannot be converted to the enum type.");
      }
      ...
      {
        return xml.HasChild(childName) ? xml[childName].InnerText : defaultValue;
      }
      ...
      {
        return xml.HasChild(childName) ? xml[childName].InnerInt() : defaultValue;
      }
      ...
      {
        return xml.HasChild(childName) ? xml[childName].InnerFloat() : defaultValue;
      }
      ...
      {
        return xml.HasChild(childName) ? xml[childName].InnerBool() : defaultValue;
      }
      ...
      {
        return xml["node"] == null ? Vector2.Zero : new Vector2((float) (int) xml["node"].AttrFloat("x"), (float) (int) xml["node"].AttrFloat("y"));
      }

      public static Vector2? FirstNodeNullable(this XmlElement xml)
      {
        return xml["node"] == null ? new Vector2?() : new Vector2?(new Vector2((float) (int) xml["node"].AttrFloat("x"), (float) (int) xml["node"].AttrFloat("y")));
      }

      public static Vector2? FirstNodeNullable(this XmlElement xml, Vector2 offset)
      {
        return xml["node"] == null ? new Vector2?() : new Vector2?(new Vector2((float) (int) xml["node"].AttrFloat("x"), (float) (int) xml["node"].AttrFloat("y")) + offset);
      }
      ...
      {
        return xml.Nodes().Length > nodeNum ? new Vector2?(xml.Nodes()[nodeNum]) : new Vector2?();
      }
      ...
      public static void EndTimer()
      {
        if (Calc.stopwatch == null)
          return;
        Calc.stopwatch.Stop();
        long ticks = Calc.stopwatch.ElapsedTicks;
        double seconds = TimeSpan.FromTicks(ticks).TotalSeconds;
        string output = $"Timer: {ticks} ticks, or {seconds.ToString("00.0000000")} seconds";
        Calc.stopwatch = (Stopwatch) null;
      }

      public static Delegate GetMethod<T>(object obj, string method) where T : class
      {
        return obj.GetType().GetMethod(method, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic) == (MethodInfo) null ? (Delegate) null : Delegate.CreateDelegate(typeof (T), obj, method);
      }

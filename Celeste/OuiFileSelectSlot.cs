...
namespace Celeste
{
    ...
    public class OuiFileSelectSlot : Entity
    {
      ...
      private void Setup()
      {
        ...
        this.Portrait.Scale = Vector2.One * (200f / (float) GFX.PortraitsSpriteBank.SpriteData[str].Sources[0].XML.AttrInt("size", 160 /*0xA0*/));
      ...
      {
        return !this.selected ? new Vector2(960f, this.Y) + new Vector2((float) x, (float) y) * new Vector2(1920f, 1080f) : new Vector2(1920f, 1080f) / 2f + new Vector2((float) x, (float) y) * new Vector2(1920f, 1080f);
      }
      ...
      public override void Render()
      {
        ...
        if (!this.Corrupted)
        {
          if (this.Exists)
          {
            ...
            if (this.SaveData.TotalHeartGems >= 16 /*0x10*/)
          ...
          if (this.Exists || this.Renaming || (double) this.newgameFade > 0.0)
          {
            ...
            Vector2 position4 = position3 + new Vector2(x1, (float) ((this.Exists ? 0 : 64 /*0x40*/) - 32 /*0x20*/));

...
namespace Celeste
{
    ...
    public class LightningRenderer : Entity
    {
      ...
      public override void Awake(Scene scene)
      {
        ...
        {
          this.bolts.Add(new LightningRenderer.Bolt(this.electricityColors[0], 1f, 160 /*0xA0*/, 160 /*0xA0*/));
          this.bolts.Add(new LightningRenderer.Bolt(this.electricityColors[1], 0.35f, 160 /*0xA0*/, 160 /*0xA0*/));
        }
      }
        ...
        Color color)
      {
        ...
        Vector2 start = a;
        int num2 = 1;
        double num3 = (double) LightningRenderer.PseudoRandRange(ref seed, 0.0f, 6.28318548f);
        float num4 = 0.0f;
        do
        {
      ...
      {
        return min + (float) ((double) (LightningRenderer.PseudoRand(ref seed) & 1023U /*0x03FF*/) / 1024.0 * ((double) max - (double) min));
      }
      ...
      private class Bolt
      {
        ...
        private IEnumerator Run()
        {
          ...
          while (true)
          {
            ...
            {
              Vector2 vector2_1 = Calc.Random.Choose<Vector2>(new Vector2(0.0f, (float) Calc.Random.Range(8, this.height - 16 /*0x10*/)), new Vector2((float) Calc.Random.Range(8, this.width - 16 /*0x10*/), 0.0f), new Vector2((float) this.width, (float) Calc.Random.Range(8, this.height - 16 /*0x10*/)), new Vector2((float) Calc.Random.Range(8, this.width - 16 /*0x10*/), (float) this.height));

...
namespace Celeste {
    ...
    public class CS10_Farewell : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            {
                int currentAnimationFrame = cs10Farewell.granny.Sprite.CurrentAnimationFrame;
                if (anim == "walk" && currentAnimationFrame == 2)
                {
                    float volume = Calc.ClampedMap((cs10Farewell.player.Position - cs10Farewell.granny.Position).Length(), 64f, 128f, 1f, 0f);
                    Audio.Play("event:/new_content/char/granny/cane_tap_ending", cs10Farewell.granny.Position).setVolume(volume);
                }
            };

...
namespace Celeste
{
    ...
    public class Spikes : Entity
    {
      ...
      public override void Added(Scene scene)
      {
        ...
        {
          for (int i = 0; i < this.size / 16 /*0x10*/; ++i)
            this.AddTentacle((float) i);
          if (this.size / 8 % 2 != 1)
            return;
          this.AddTentacle((float) (this.size / 16 /*0x10*/) - 0.5f);
        }
        else
        {
      ...
      {
        return this.Direction == Spikes.Directions.Up && this.CollideCheck((Entity) jumpThru, this.Position + Vector2.UnitY);
      }

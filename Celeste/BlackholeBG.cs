...
public class BlackholeBG : Backdrop
{
  ...
  public BlackholeBG()
  {
    ...
    for (int index2 = 0; index2 < 50; ++index2)
    {
      ...
      this.streams[index2].Normal = Calc.AngleToVector(Calc.Random.NextFloat() * 6.28318548f, 1f);
    ...
    for (int index4 = 0; index4 < 10; ++index4)
    {
      ...
      this.spirals[index4].Offset = Calc.Random.NextFloat(6.28318548f);
    ...
    for (int index6 = 0; index6 < 220; ++index6)
    {
      ...
      this.particles[index6].Normal = Calc.AngleToVector(Calc.Random.NextFloat() * 6.28318548f, 1f);
  ...
  public int StreamCount
  {
    get
    {
      return (int) MathHelper.Lerp(30f, 50f, (float) (((double) this.StrengthMultiplier - 1.0) / 3.0));
    }
  }
  ...
  public int ParticleCount
  {
    get
    {
      return (int) MathHelper.Lerp(150f, 220f, (float) (((double) this.StrengthMultiplier - 1.0) / 3.0));
    }
  }
  ...
  public int SpiralCount
  {
    get
    {
      return (int) MathHelper.Lerp(0.0f, 10f, (float) (((double) this.StrengthMultiplier - 1.0) / 3.0));
    }
  }
  ...
  public override void Update(Scene scene)
  {
    ...
    for (int index = 0; index < streamCount; ++index)
    {
      ...
      {
        this.streams[index].Normal = Calc.AngleToVector(Calc.Random.NextFloat() * 6.28318548f, 1f);
      ...
      Color color1 = this.colorsLerpBlack[this.streams[index].Color, (int) ((double) num2 * 0.60000002384185791 * 19.0)];
      Vector2 vector2_3 = normal * 16f + normal * (1f - num3) * 280f;
      float num5 = (float) ((1.0 - (double) num3) * 8.0);
      Color color2 = this.colorsLerpBlack[this.streams[index].Color, (int) ((double) num3 * 0.60000002384185791 * 19.0)];
    ...
    for (int index = 0; index < particleCount; ++index)
    {
      ...
      {
        this.particles[index].Normal = Calc.AngleToVector(Calc.Random.NextFloat() * 6.28318548f, 1f);
    ...
    for (int index3 = 0; index3 < spiralCount; ++index3)
    {
      ...
      {
        this.spirals[index3].Offset = Calc.Random.NextFloat(6.28318548f);
    ...
    this.center += (new Vector2(320f, 180f) / 2f + wind * 0.15f + this.CenterOffset - this.center) * (1f - (float) Math.Pow(0.0099999997764825821, (double) Engine.DeltaTime));
    this.offset += (-wind * 0.25f + this.OffsetOffset - this.offset) * (1f - (float) Math.Pow(0.0099999997764825821, (double) Engine.DeltaTime));
    if (scene.OnInterval(0.025f))
      this.shake = Calc.AngleToVector(Calc.Random.NextFloat(6.28318548f), (float) (2.0 * ((double) this.StrengthMultiplier - 1.0)));

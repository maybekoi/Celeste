    ...
    public static class Commands
    {
      ...
      private static void CmdDetailedLevels(int area = -1, int mode = 0)
      {
        if (area == -1)
        {
          ...
          Engine.Commands.Log((object) $"{(object) num2} / {(object) num1}");
        }
        else
        {
          int num3 = 0;
          ...
          Engine.Commands.Log((object) $"{(object) num4} / {(object) num3}");
        }
      }
      ...
      {
        Engine.Scene = (Scene) new PreviewPostcard(new Postcard(Dialog.Get("POSTCARD_" + id), area));
      }

      [Command("postcard_cside", "views a postcard")]
      private static void CmdPostcardCside()
      {
        Engine.Scene = (Scene) new PreviewPostcard(new Postcard(Dialog.Get("POSTCARD_CSIDES"), "event:/ui/main/postcard_csides_in", "event:/ui/main/postcard_csides_out"));
      }

      [Command("postcard_variants", "views a postcard")]
      private static void CmdPostcardVariants()
      {
        Engine.Scene = (Scene) new PreviewPostcard(new Postcard(Dialog.Get("POSTCARD_VARIANTS"), "event:/new_content/ui/postcard_variants_in", "event:/new_content/ui/postcard_variants_out"));
      }
          ...
          for (int index1 = 0; index1 < area.Mode.Length; ++index1)
          {
            if (area.Mode[index1] != null)
            {
              ...
              foreach (EntityData strawberry in area.Mode[index1].MapData.Strawberries)
              {
                ...
                string str = $"{(object) index2}:{(object) index3}";
      ...
      {
        Engine.Scene = (Scene) new OverworldReflectionsFall((Level) null, (Action) (() => Engine.Scene = (Scene) new OverworldLoader(Overworld.StartMode.Titlescreen)));
      }
      ...
      private static void CmdGiveKey()
      {
        ...
        Key key = new Key(entity, new EntityID("unknown", 1073741823 /*0x3FFFFFFF*/ + Calc.Random.Next(10000)));

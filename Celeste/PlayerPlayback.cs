    ...
    public class PlayerPlayback : Entity
    {
      ...
      public override void Update()
      {
        ...
        if (this.index >= this.Timeline.Count - 1 || (double) this.Time >= (double) this.TrimEnd)
        {
          ...
          {
            Player entity = this.Scene == null ? (Player) null : this.Scene.Tracker.GetEntity<Player>();
            if (entity == null || (double) entity.X > (double) this.rangeMinX && (double) entity.X < (double) this.rangeMaxX)
              this.Restart();
          }
        }

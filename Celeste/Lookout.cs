...
namespace Celeste
{
    ...
    public class Lookout : Entity
    {
        ...
        private IEnumerator LookRoutine(Player player)
        {
            ...
            if (Math.Abs(lookout.X - player.X) > 4.0 || player.Dead || !player.OnGround())
            {
                if (!player.Dead)
            ...
            else
            {
                ...
                while ((lookout.hud.Easer = Calc.Approach(lookout.hud.Easer, 1f, Engine.DeltaTime * 3f)) < 1.0)
                {
                    level.ScreenPadding = (int)(Ease.CubeInOut(lookout.hud.Easer) * 16.0);
                    yield return null;
                }
                float accel = 800f;
                float maxspd = 240f;
                ...
                while (!Input.MenuCancel.Pressed && !Input.MenuConfirm.Pressed && !Input.Dash.Pressed && !Input.Jump.Pressed && lookout.interacting)
                {
                    ...
                    label_47:
                        ...
                        double num1 = Calc.Clamp((float)x, (float)left, (float)max1);
                        cam.X = (float)num1;
                        foreach (Entity entity in entities)
                        {
                            if (cam.X + 320.0 > entity.Left && cam.Y + 180.0 > entity.Top && cam.X < (double)entity.Right && cam.Y < (double)entity.Bottom)
                            {
                                cam.X = vector2_2.X;
                                speed.X = 0.0f;
                            }
                    ...
                    label_56:
                        ...
                        double num3 = Calc.Clamp((float)y, (float)top, (float)max2);
                        cam.Y = (float)num3;
                        foreach (Entity entity in entities)
                        {
                            if (cam.X + 320.0 > entity.Left && cam.Y + 180.0 > entity.Top && cam.X < (double)entity.Right && cam.Y < (double)entity.Bottom)
                ...
                while ((lookout.hud.Easer = Calc.Approach(lookout.hud.Easer, 0.0f, Engine.DeltaTime * 3f)) > 0.0)
                {
                    level.ScreenPadding = (int)(Ease.CubeInOut(lookout.hud.Easer) * 16.0);
                    yield return null;
                }
                ...
                if ((camStart - level.Camera.Position).Length() > 600.0)
                {
                    ...
                    for (duration = 0.0f; duration < 1.0; duration += Engine.DeltaTime / approach)
                    {
        ...
        private class Hud : Entity
        {
            ...
            public Hud() => AddTag((int)Tags.HUD);
            ...
            public override void Update()
            {
                ...
                int width = 320;
                int height = 180;
                bool collideLeft = Scene.CollideCheck<LookoutBlocker>(new Rectangle((int)(position.X - 8.0), (int)position.Y, width, height));
                bool collideRight = Scene.CollideCheck<LookoutBlocker>(new Rectangle((int)(position.X + 8.0), (int)position.Y, width, height));
                bool collideUp = TrackMode && TrackPercent >= 1.0 || Scene.CollideCheck<LookoutBlocker>(new Rectangle((int)position.X, (int)(position.Y - 8.0), width, height));
                bool collideDown = TrackMode && TrackPercent <= 0.0 || Scene.CollideCheck<LookoutBlocker>(new Rectangle((int)position.X, (int)(position.Y + 8.0), width, height));
                left = Calc.Approach(left, collideLeft || position.X <= (double)(bounds.Left + 2) ? 0.0f : 1f, Engine.DeltaTime * 8f);
                right = Calc.Approach(right, collideRight || position.X + (double)width >= bounds.Right - 2 ? 0.0f : 1f, Engine.DeltaTime * 8f);
                up = Calc.Approach(up, collideUp || position.Y <= (double)(bounds.Top + 2) ? 0.0f : 1f, Engine.DeltaTime * 8f);
                down = Calc.Approach(down, collideDown || position.Y + (double)height >= bounds.Bottom - 2 ? 0.0f : 1f, Engine.DeltaTime * 8f);
                aim = Input.Aim.Value;
                if (aim.X < 0.0)
                {
            ...
            public override void Render()
            {
                ...
                int x1 = (int)(80.0 * num1);
                int y1 = (int)(80.0 * num1 * (9.0 / 16.0));
                int height = 8;
                ...
                float y2 = (float)(y1 * (double)up - (float)(Math.Sin(timerUp) * 18.0) * MathHelper.Lerp(0.5f, 1f, multUp) - (1.0 - multUp) * 12.0);
                arrow.DrawCentered(new Vector2(960f, y2), color * up, 1f, 1.57079637f);
                float y3 = (float)(1080.0 - y1 * (double)down + (float)(Math.Sin(timerDown) * 18.0) * MathHelper.Lerp(0.5f, 1f, multDown) + (1.0 - multDown) * 12.0);
                arrow.DrawCentered(new Vector2(960f, y3), color * down, 1f, 4.712389f);
                if (!TrackMode && !OnlyY)
                {
                    float num2 = left;
                    ...
                    float x2 = (float)(x1 * (double)num2 - (float)(Math.Sin(a1) * 18.0) * MathHelper.Lerp(0.5f, 1f, amount1) - (1.0 - amount1) * 12.0);
                    arrow.DrawCentered(new Vector2(x2, 540f), color * num2);
                    float x3 = (float)(1920.0 - x1 * (double)num3 + (float)(Math.Sin(a2) * 18.0) * MathHelper.Lerp(0.5f, 1f, amount2) + (1.0 - amount2) * 12.0);
                    arrow.DrawCentered(new Vector2(x3, 540f), color * num3, 1f, 3.14159274f);
                }
                else
                {
                    if (!TrackMode)

...
public class BreathingMinigame : Entity
{
  ...
  public IEnumerator Routine()
  {
    ...
    while ((double) breathingMinigame.stablizedTimer < 30.0)
    {
      ...
      float target = (float) (0.64999997615814209 + (double) Math.Min(1f, t / 0.8f) * 0.35000002384185791);
    ...
    if (!breathingMinigame.winnable)
    {
      ...
      while (breathingMinigame.Pausing)
      {
        ...
        featherSprite.Position = featherSprite.Position + (breathingMinigame.screenCenter - breathingMinigame.featherSprite.Position) * (1f - (float) Math.Pow(0.0099999997764825821, (double) Engine.DeltaTime));
  ...
  public override void Render()
  {
    ...
    if ((double) this.particleSpeed < 0.0)
      vector2_1 = new Vector2(Math.Min(1f, (float) (1.0 / (-(double) this.particleSpeed * 0.0040000001899898052))), Math.Max(1f, (float) (1.0 * -(double) this.particleSpeed * 0.0040000001899898052)));
    for (int index = 0; index < this.particles.Length; ++index)

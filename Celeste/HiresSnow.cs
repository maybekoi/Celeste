    ...
    public class HiresSnow : Monocle.Renderer
    {
      ...
      public override void Update(Scene scene)
      {
        ...
        for (int index = 0; index < this.particles.Length; ++index)
        {
          ...
          if ((double) this.particles[index].Position.X < (double) sbyte.MinValue || (double) this.particles[index].Position.X > (double) (Engine.Width + 128 /*0x80*/) || (double) this.particles[index].Position.Y < (double) sbyte.MinValue || (double) this.particles[index].Position.Y > (double) (Engine.Height + 128 /*0x80*/))
            this.particles[index].Reset(this.Direction);
        }
      ...
      private struct Particle
      {
        ...
        public void Reset(Vector2 direction)
        {
          ...
          if ((double) direction.X < 0.0)
            this.Position = new Vector2((float) (Engine.Width + 128 /*0x80*/), Calc.Random.NextFloat((float) Engine.Height));
            ...
            this.Position = new Vector2(Calc.Random.NextFloat((float) Engine.Width), (float) (Engine.Height + 128 /*0x80*/));

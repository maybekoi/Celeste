...
public class PreviewDialog : Scene
{
        ...
        if (textbox1 != null)
        {
          for (int index = textbox1.Start + 1; index <= textbox2.Start && index < textbox1.Nodes.Count; ++index)
          {
            if (textbox1.Nodes[index] is FancyText.Trigger node)
              this.elements.Add((object) $"{(node.Silent ? (object) "Silent " : (object) "")}Trigger [{(object) node.Index}] {node.Label}");
          }
        }
  ...
  {
    return (double) this.Mouse.X > (double) this.listScroll.X && (double) this.Mouse.Y > (double) this.listScroll.Y + (double) i * (double) ActiveFont.LineHeight * 0.60000002384185791 && (double) MInput.Mouse.X < 960.0 && (double) this.Mouse.Y < (double) this.listScroll.Y + (double) (i + 1) * (double) ActiveFont.LineHeight * 0.60000002384185791;
  }

...
public class WaveDashPresentation : Entity
{
  ...
  {
    get => this.pageIndex <= 0 ? (WaveDashPage) null : this.pages[this.pageIndex - 1];
  }

  private WaveDashPage CurrPage
  {
    get => this.pageIndex >= this.pages.Count ? (WaveDashPage) null : this.pages[this.pageIndex];
  }
  ...
  private void BeforeRender()
  {
    ...
    if (this.pageTurning)
    {
      ...
      {
        float rotation = -1.57079637f * this.pageEase;
        this.RenderQuad((Texture) (RenderTarget2D) this.prevPageBuffer, this.pageEase, rotation);
        this.RenderQuad((Texture) (RenderTarget2D) this.currPageBuffer, this.pageEase, 1.57079637f + rotation);
      }
      else if (this.CurrPage.Transition == WaveDashPage.Transitions.Blocky)
      {
        Draw.SpriteBatch.Begin();
  ...
  private void RenderQuad(Texture texture, float ease, float rotation)
  {
    ...
    float num2 = (float) (4.1500000953674316 + (double) Calc.YoYo(ease) * 1.7000000476837158);
  ...
  {
    return min + (float) ((double) (WaveDashPresentation.PseudoRand(ref seed) % 1000U) / 1000.0 * ((double) max - (double) min));
  }
}

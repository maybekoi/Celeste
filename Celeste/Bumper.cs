...
public class Bumper : Entity
{
  ...
  public Bumper(Vector2 position, Vector2? node)
    : base(position)
  {
    this.Collider = (Collider) new Monocle.Circle(12f);
    ...
    this.Add((Component) (this.light = new VertexLight(Color.Teal, 1f, 16 /*0x10*/, 32 /*0x20*/)));
    ...
    this.UpdatePosition();
    this.Add((Component) (this.hitWiggler = Wiggler.Create(1.2f, 2f, (Action<float>) (v => this.spriteEvil.Position = this.hitDir * this.hitWiggler.Value * 8f))));
  ...
  public override void Update()
  {
    ...
    else if (this.Scene.OnInterval(0.05f))
    {
      ...
      float direction = this.fireMode ? -1.57079637f : angleRadians;
  ...
  private void OnPlayer(Player player)
  {
    ...
    else
    {
      ...
      this.spriteEvil.Play("hit", true);
      this.light.Visible = false;
      this.bloom.Visible = false;
      this.SceneAs<Level>().DirectionalShake(vector2, 0.15f);
      this.SceneAs<Level>().Displacement.AddBurst(this.Center, 0.3f, 8f, 32f, 0.8f);

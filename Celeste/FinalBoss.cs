...
namespace Celeste
{
    ...
    public class FinalBoss : Entity
    {
      ...
      {
        get => this.Center + this.Sprite.Position + new Vector2(6f * this.Sprite.Scale.X, 2f);
      }
      ...
      private IEnumerator MoveSequence(Player player, bool lastHit)
      {
        ...
        {
          Vector2 position = finalBoss.Center + finalBoss.Sprite.Position + Calc.AngleToVector(direction + Calc.Random.Range(-1f * (float) Math.PI / 90f, (float) Math.PI / 90f), (float) Calc.Random.Range(16 /*0x10*/, 20));

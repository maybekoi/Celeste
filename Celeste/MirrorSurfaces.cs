...
{
  public const int MaxMirrorOffset = 32 /*0x20*/;
  ...
  public void BeforeRender()
  {
    ...
    if (!(this.hasReflections = components2.Count > 0 && components1.Count > 0))
      return;
    if (this.target == null)
      this.target = VirtualContent.CreateRenderTarget("mirror-surfaces", 320, 180);
    ...
    Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None, RasterizerState.CullNone, (Effect) null, transformMatrix);
    ...
    Engine.Graphics.GraphicsDevice.Clear(Color.Transparent);
    Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, (DepthStencilState) null, RasterizerState.CullNone, (Effect) null, transformMatrix);

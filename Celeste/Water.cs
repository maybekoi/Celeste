...
public class Water : Entity
{
  ...
  public Water(EntityData data, Vector2 offset)
    : this(data.Position + offset, true, data.Bool("hasBottom"), (float) data.Width, (float) data.Height)
  {
  }
  ...
  public Water(Vector2 position, bool topSurface, bool bottomSurface, float width, float height)
  {
    ...
    if (topSurface)
    {
      this.TopSurface = new Water.Surface(this.Position + new Vector2(width / 2f, (float) y), new Vector2(0.0f, -1f), width, height);
    ...
    if (bottomSurface)
    {
      this.BottomSurface = new Water.Surface(this.Position + new Vector2(width / 2f, height - (float) y), new Vector2(0.0f, 1f), width, height);
  ...
  public override void Update()
  {
    ...
    foreach (WaterInteraction component in this.Scene.Tracker.GetComponents<WaterInteraction>())
    {
      ...
      if (flag1 != flag2)
      {
        ...
        int num = (double) entity.Center.Y >= (double) this.Center.Y || this.Scene.CollideCheck<Solid>(new Rectangle((int) entity.Center.X - 4, (int) entity.Center.Y, 8, 16 /*0x10*/)) ? 0 : 1;
        if (flag1)
        {
          if (flag3)
      ...
      if (this.BottomSurface != null && entity is Player)
      {
        if (flag2 && (double) entity.Y > (double) this.Bottom - 8.0)
        {
          ...
          this.playerBottomTension.Strength = Calc.ClampedMap(entity.Y, this.Bottom - 8f, this.Bottom + 4f);
        }
  ...
  public class Ray
  {
    ...
    public void Reset(float percent)
    {
      ...
      this.Width = (float) Calc.Random.Range(2, 16 /*0x10*/);
  ...
  public class Surface
  {
    ...
    {
      Vector2 vector2 = this.Outwards.Perpendicular();
      Vector2 lineA = this.Position + vector2 * (float) (-this.Width / 2);
      return (lineA - Calc.ClosestPointOnLine(lineA, this.Position + vector2 * (float) (this.Width / 2), position)).Length();
    }
    ...
    public void RemoveTension(Water.Tension tension) => this.Tensions.Remove(tension);
    ...
    public void DoRipple(Vector2 position, float multiplier)
    {
      ...
      if (this.Width < 200)
      {
        num2 *= Calc.ClampedMap((float) this.Width, 0.0f, 200f, 0.25f);
        multiplier *= Calc.ClampedMap((float) this.Width, 0.0f, 200f, 0.5f);
      }
    ...
    public void Update()
    {
      ...
      {
        int position1 = num1;
        float surfaceHeight1 = this.GetSurfaceHeight((float) position1);
        int position2 = Math.Min(num1 + 4, this.Width);
        float surfaceHeight2 = this.GetSurfaceHeight((float) position2);
        this.mesh[fillStartIndex].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position1) + this.Outwards * surfaceHeight1, 0.0f);
        this.mesh[fillStartIndex + 1].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position2) + this.Outwards * surfaceHeight2, 0.0f);
        this.mesh[fillStartIndex + 2].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position1), 0.0f);
        this.mesh[fillStartIndex + 3].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position2) + this.Outwards * surfaceHeight2, 0.0f);
        this.mesh[fillStartIndex + 4].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position2), 0.0f);
        this.mesh[fillStartIndex + 5].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position1), 0.0f);
        this.mesh[surfaceStartIndex].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position1) + this.Outwards * (surfaceHeight1 + 1f), 0.0f);
        this.mesh[surfaceStartIndex + 1].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position2) + this.Outwards * (surfaceHeight2 + 1f), 0.0f);
        this.mesh[surfaceStartIndex + 2].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position1) + this.Outwards * surfaceHeight1, 0.0f);
        this.mesh[surfaceStartIndex + 3].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position2) + this.Outwards * (surfaceHeight2 + 1f), 0.0f);
        this.mesh[surfaceStartIndex + 4].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position2) + this.Outwards * surfaceHeight2, 0.0f);
        this.mesh[surfaceStartIndex + 5].Position = new Vector3(this.Position + vector2_1 * (float) (-this.Width / 2 + position1) + this.Outwards * surfaceHeight1, 0.0f);
        num1 += 4;
        fillStartIndex += 6;
        surfaceStartIndex += 6;
      }
      ...
      foreach (Water.Ray ray in this.Rays)
      {
        ...
        float num2 = 1f;
        if ((double) ray.Percent < 0.10000000149011612)
          num2 = Calc.ClampedMap(ray.Percent, 0.0f, 0.1f);
        else if ((double) ray.Percent > 0.89999997615814209)
          num2 = Calc.ClampedMap(ray.Percent, 0.9f, 1f, 1f, 0.0f);
        float position3 = Math.Max(0.0f, ray.Position - ray.Width / 2f);
        float position4 = Math.Min((float) this.Width, ray.Position + ray.Width / 2f);
        float num3 = Math.Min((float) this.BodyHeight, 0.7f * ray.Length);
        float num4 = 0.3f * ray.Length;
        Vector2 vector2_3 = vector2_2 + vector2_1 * position3 + this.Outwards * this.GetSurfaceHeight(position3);
        Vector2 vector2_4 = vector2_2 + vector2_1 * position4 + this.Outwards * this.GetSurfaceHeight(position4);
        Vector2 vector2_5 = vector2_2 + vector2_1 * (position4 - num4) - this.Outwards * num3;
        Vector2 vector2_6 = vector2_2 + vector2_1 * (position3 - num4) - this.Outwards * num3;
    ...
    public float GetSurfaceHeight(float position)
    {
      ...
      return num3 * Calc.ClampedMap(val1, 0.0f, 0.1f, 0.5f) * Calc.ClampedMap(val1, 0.9f, 1f, 1f, 0.5f) + (float) Math.Sin((double) this.timer + (double) position * 0.10000000149011612) + 6f;
    }

    public void Render(Camera camera)
    {
      GFX.DrawVertices<VertexPositionColor>(camera.Matrix, this.mesh, this.mesh.Length);
    }
  }
}

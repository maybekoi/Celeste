...
public class TrailManager : Entity
{
  ...
  private const int size = 64 /*0x40*/;
  private const int columns = 8;
  private const int rows = 8;
  private TrailManager.Snapshot[] snapshots = new TrailManager.Snapshot[64 /*0x40*/];
  ...
  private void BeforeRender()
  {
    if (!this.dirty)
      return;
    if (this.buffer == null)
      this.buffer = VirtualContent.CreateRenderTarget("trail-manager", 512 /*0x0200*/, 512 /*0x0200*/);
    ...
    for (int index = 0; index < this.snapshots.Length; ++index)
    {
      if (this.snapshots[index] != null && !this.snapshots[index].Drawn)
        Draw.Rect((float) (index % 8 * 64 /*0x40*/), (float) (index / 8 * 64 /*0x40*/), 64f, 64f, Color.Transparent);
    }
  ...
  {
    TrailManager.Add(position, image, (PlayerHair) null, image.Scale, color, depth, duration);
  }
  ...
  public class Snapshot : Entity
  {
    ...
    public override void Render()
    {
      ...
      Rectangle rectangle = new Rectangle(this.Index % 8 * 64 /*0x40*/, this.Index / 8 * 64 /*0x40*/, 64 /*0x40*/, 64 /*0x40*/);

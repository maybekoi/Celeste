...
namespace Celeste
{
    ...
    public class Credits
    {
      ...
      private static List<Credits.CreditNode> CreateCredits(bool title, bool polaroids)
      {
        ...
        credits.Add((Credits.CreditNode) new Credits.Thanks("Contributors & Playtesters", new string[32 /*0x20*/]
        {
          "Nels Anderson",
          "Liam & Graeme Berry",
          "Tamara Bruketta",
      ...
      private class Role : Credits.CreditNode
      {
        ...
        {
          return (float) ((double) Credits.LineHeight * 2.7999999523162842 + 8.0 + 64.0) * scale;
        }
      }
      ...
      private class Team : Credits.CreditNode
      {
        ...
        public override float Height(float scale = 1f)
        {
          return (float) ((double) Credits.LineHeight * (1.7999999523162842 + (double) this.Members.Length * 1.3999999761581421 + 1.0) + 8.0 + 64.0) * scale;
        }
      }
      ...
      private class Thanks : Credits.CreditNode
      {
        ...
        public override float Height(float scale = 1f)
        {
          return ((float) ((double) Credits.LineHeight * (1.3999999761581421 + (double) this.Credits.Length * 1.1499999761581421) + (this.Credits.Length != 0 ? 8.0 : 0.0)) + (float) this.TopPadding) * scale;
        }
      }
      ...
      private class Ending : Credits.CreditNode
      {
        ...
        public override float Height(float scale = 1f)
        {
          return this.Spacing ? 540f : ActiveFont.LineHeight * 1.5f * scale;
        }
      }
      ...
      private class ImageRow : Credits.CreditNode
      {
        ...
        public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
        {
          ...
          foreach (Credits.Image image in this.images)
            num2 += (float) (image.Atlas[image.ImagePath].Width + 32 /*0x20*/) * scale;
          ...
          foreach (Credits.Image image in this.images)
          {
            ...
            vector2.X += (float) (image.Atlas[image.ImagePath].Width + 32 /*0x20*/) * scale;
          }
        }

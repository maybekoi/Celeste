...
public class SwapBlock : Solid
{
    ...
    : base(position, width, height, false)
  {
    ...
    {
      this.Add((Component) (this.middleGreen = GFX.SpriteBank.Create("swapBlockLightMoon")));
      this.Add((Component) (this.middleRed = GFX.SpriteBank.Create("swapBlockLightRedMoon")));
    }
  ...
  public SwapBlock(EntityData data, Vector2 offset)
    : this(data.Position + offset, (float) data.Width, (float) data.Height, data.Nodes[0] + offset, data.Enum<SwapBlock.Themes>("theme"))
  {
  }
  ...
  {
    base.Removed(scene);
    Audio.Stop(this.moveSfx);
    Audio.Stop(this.returnSfx);
  }
  ...
  private void OnDash(Vector2 direction)
  {
    ...
    this.burst = (this.Scene as Level).Displacement.AddBurst(this.Center, 0.2f, 0.0f, 16f);
    this.speed = (double) this.lerp < 0.20000000298023224 ? MathHelper.Lerp(this.maxForwardSpeed * 0.333f, this.maxForwardSpeed, this.lerp / 0.2f) : this.maxForwardSpeed;
    Audio.Stop(this.returnSfx);
    Audio.Stop(this.moveSfx);
    if (!this.Swapping)
    ...
    Color color)
  {
    ...
    for (int x = 1; x < num1 - 1; ++x)
    {
      for (int y = 1; y < num2 - 1; ++y)
        ninSlice[1, 1].Draw(pos + new Vector2((float) x, (float) y) * 8f, Vector2.Zero, color);
    }
    if (middle == null)
      return;
    middle.Color = color;

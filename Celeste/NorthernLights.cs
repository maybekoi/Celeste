...
public class NorthernLights : Backdrop
{
  ...
  private VertexPositionColorTexture[] verts = new VertexPositionColorTexture[1024 /*0x0400*/];
  ...
  {
    if (this.buffer == null)
      this.buffer = VirtualContent.CreateRenderTarget("northern-lights", 320, 180);
    int vert = 0;
    foreach (NorthernLights.Strand strand in this.strands)
    {
    ...
    GFX.DrawVertices<VertexPositionColor>(Matrix.Identity, this.gradient, this.gradient.Length);
    ...
    GFX.DrawVertices<VertexPositionColorTexture>(Matrix.Identity, this.verts, vert, GFX.FxTexture);
    bool clear = false;
    GaussianBlur.Blur((Texture2D) (RenderTarget2D) this.buffer, GameplayBuffers.TempA, this.buffer, clear: clear, samples: GaussianBlur.Samples.Five, sampleScale: 0.25f, direction: GaussianBlur.Direction.Vertical);
  ...
  private class Strand
  {
    ...
    public Strand() => this.Reset(Calc.Random.NextFloat());
    ...
    public void Reset(float startPercent)
    {
      ...
      for (int index = 0; index < 40; ++index)
      {
        NorthernLights.Node node = new NorthernLights.Node()
        {
          Position = vector2,
          TextureOffset = num,
          Height = (float) Calc.Random.Range(10, 80 /*0x50*/),

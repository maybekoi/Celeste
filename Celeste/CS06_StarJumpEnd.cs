...
namespace Celeste {
    ...
    public class CS06_StarJumpEnd : CutsceneEntity
    {
        ...
        {
            base.Update();
            if (this.waiting && (double)this.player.Y <= (double)(this.Level.Bounds.Top + 160 /*0xA0*/))
            {
                this.waiting = false;
                this.Start();
            }
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            int center = level.Bounds.X + 160 /*0xA0*/;
            ...
            for (p = 0.0f; (double)p < 1.0; p += Engine.DeltaTime / 3f)
            {
                ...
                if ((double)cs06StarJumpEnd.player.X < (double)(level.Bounds.X + 32 /*0x20*/))
                    cs06StarJumpEnd.player.X = (float)(level.Bounds.X + 32 /*0x20*/);
                Rectangle bounds;
                if ((double)cs06StarJumpEnd.player.X > (double)(level.Bounds.Right - 32 /*0x20*/))
                {
                    Player player = cs06StarJumpEnd.player;
                    bounds = level.Bounds;
                    double num = (double)(bounds.Right - 32 /*0x20*/);
                    player.X = (float)num;
                }
        ...
        private IEnumerator TentaclesGrab()
        {
            ...
            */
            ...
            cs06StarJumpEnd.Level.Shake();
            cs06StarJumpEnd.rumbler = new BreathingRumbler();
            cs06StarJumpEnd.Level.Add(cs06StarJumpEnd.rumbler);
            yield break;
        }
        ...
        private IEnumerator StartCirclingPlayer()
        {
            ...
            */
            CS06_StarJumpEnd cs06StarJumpEnd = this;
            cs06StarJumpEnd.Add(new Coroutine(cs06StarJumpEnd.BadelineCirclePlayer(), true));
            Vector2 from = cs06StarJumpEnd.player.Position;
            Vector2 to = new Vector2(cs06StarJumpEnd.Level.Bounds.Center.X, cs06StarJumpEnd.player.Y);

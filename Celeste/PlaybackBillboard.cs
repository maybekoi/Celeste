...
public class PlaybackBillboard : Entity
{
  ...
  {
    return !this.Scene.CollideCheck<PlaybackBillboard>(new Rectangle((int) this.X + x * 8, (int) this.Y + y * 8, 8, 8));
  }
  ...
  {
    return min + (float) ((double) (PlaybackBillboard.PseudoRand(ref seed) % 1000U) / 1000.0 * ((double) max - (double) min));
  }

...
namespace Celeste {
    ...
    public class OuiChapterPanel : Oui
    {
        ...
        {
            get => this.Position + new Vector2(this.contentOffset.X, 128f + this.height);
        }
        ...
        {
            get => !this.selectingMode ? this.checkpoints : this.modes;
        }
        ...
        private IEnumerator SwapRoutine()
        {
            ...
            if (!ouiChapterPanel.selectingMode)
            {
                ...
                ouiChapterPanel.checkpoints.Add(new OuiChapterPanel.Option()
                {
                    ...
                    CheckpointOffset = new Vector2((float)Calc.Random.Range(-16, 16 /*0x10*/), (float)Calc.Random.Range(-16, 16 /*0x10*/)),
                    Large = false,
                    Siblings = num
                });
        ...
        public override void Render()
        {
            ...
            this.card.Draw(this.Position + new Vector2(0.0f, (float)(mtexture1.Height - 32 /*0x20*/)));
            for (int index = 0; index < this.options.Count; ++index)
            {
                if (this.options[index].OnTopOfUI)
                    this.options[index].Render(optionsRenderPosition, this.option == index, this.wiggler, this.modeAppearWiggler);
        ...
        {
            return snap ? to : (from += (to - from) * (1f - (float)Math.Pow(1.0 / 1000.0, (double)Engine.DeltaTime)));
        }
        ...
        {
            return level == null ? area.ToString() : $"{area.ToString()}_{level}";
        }
        ...
        private class Option
        {
            ...
            public void Render(Vector2 center, bool selected, Wiggler wiggler, Wiggler appearWiggler)
            {
                ...
                Color color1 = Color.Lerp(this.BgColor, Color.Black, (float)((1.0 - (double)this.Pop) * 0.60000002384185791));

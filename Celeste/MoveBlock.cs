...
public class MoveBlock : Solid
{
    ...
    : base(position, (float) width, (float) height, false)
  {
    ...
    else if (canSteer && (direction == MoveBlock.Directions.Up || direction == MoveBlock.Directions.Down))
    {
      for (int index = 0; index < num2; ++index)
      {
        ...
        this.AddImage(mtexture2.GetSubtexture(num4 * 8, 0, 8, 8), new Vector2(-4f, (float) (index * 8)), 1.57079637f, new Vector2(1f, -1f), this.leftButton);
        this.AddImage(mtexture2.GetSubtexture(num4 * 8, 0, 8, 8), new Vector2((float) ((num1 - 1) * 8 + 4), (float) (index * 8)), 1.57079637f, new Vector2(1f, 1f), this.rightButton);
      }
    ...
    while (true)
    {
      ...
      while (true)
      {
        ...
        else
        {
          ...
          if (moveBlock.direction == MoveBlock.Directions.Down && (double) moveBlock.Top > (double) (moveBlock.SceneAs<Level>().Bounds.Bottom + 32 /*0x20*/))
            flag1 = true;
        }
        if (flag1)
        {
  ...
  private void ActivateParticles()
  {
    ...
    if (num != 0)
      this.SceneAs<Level>().ParticlesBG.Emit(MoveBlock.P_Activate, (int) ((double) this.Height / 2.0), this.CenterLeft, Vector2.UnitY * (this.Height - 4f) * 0.5f, 3.14159274f);
    if (flag2)
    ...
    if (flag3)
      this.SceneAs<Level>().ParticlesBG.Emit(MoveBlock.P_Activate, (int) ((double) this.Width / 2.0), this.TopCenter, Vector2.UnitX * (this.Width - 4f) * 0.5f, -1.57079637f);
    this.SceneAs<Level>().ParticlesBG.Emit(MoveBlock.P_Activate, (int) ((double) this.Width / 2.0), this.BottomCenter, Vector2.UnitX * (this.Width - 4f) * 0.5f, 1.57079637f);
  }
  ...
  private class Debris : Actor
  {
    ...
    public MoveBlock.Debris Init(Vector2 position, Vector2 center, Vector2 returnTo)
    {
      ...
      this.alpha = 1f;
      this.firstHit = false;
      this.spin = Calc.Random.Range(3.49065852f, 10.4719753f) * (float) Calc.Random.Choose<int>(1, -1);
      return this;
    }

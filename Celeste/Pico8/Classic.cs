...
public class Classic
{
  ...
  private bool is_title() => this.level_index() == 31 /*0x1F*/;
  ...
  private void draw_player(Classic.ClassicObject obj, int djump)
  {
    ...
    switch (djump)
    {
      case 0:
        ...
        break;
      case 2:
        num = this.E.flr((float) (this.frames / 3 % 2)) != 0 ? 144 /*0x90*/ : 160 /*0xA0*/;
        break;
    }
  ...
  public void load_room(int x, int y)
  {
    ...
    for (int index1 = 0; index1 <= 15; ++index1)
    {
      ...
      {
        int num = this.E.mget(this.room.X * 16 /*0x10*/ + index1, this.room.Y * 16 /*0x10*/ + index2);
        switch (num)
        {
          case 11:
  ...
  public void Draw()
  {
    ...
    this.E.map(this.room.X * 16 /*0x10*/, this.room.Y * 16 /*0x10*/, 0, 0, 16 /*0x10*/, 16 /*0x10*/, 2);
    for (int index = 0; index < this.objects.Count; ++index)
    {
      Classic.ClassicObject classicObject = this.objects[index];
    ...
    this.E.map(this.room.X * 16 /*0x10*/, this.room.Y * 16 /*0x10*/, this.is_title() ? -4 : 0, 0, 16 /*0x10*/, 16 /*0x10*/, 1);
    for (int index = 0; index < this.objects.Count; ++index)
    {
      Classic.ClassicObject classicObject = this.objects[index];
      switch (classicObject)
    ...
    this.E.map(this.room.X * 16 /*0x10*/, this.room.Y * 16 /*0x10*/, 0, 0, 16 /*0x10*/, 16 /*0x10*/, 3);
  ...
  private void draw_time(int x, int y)
  {
    ...
    this.E.rectfill((float) x, (float) y, (float) (x + 32 /*0x20*/), (float) (y + 6), 0.0f);
    this.E.print($"{(num2 < 10 ? (object) "0" : (object) "")}{(object) num2}:{(num1 < 10 ? (object) "0" : (object) "")}{(object) num1}:{(seconds < 10 ? (object) "0" : (object) "")}{(object) seconds}", (float) (x + 1), (float) (y + 1), 7f);
  }
  ...
  {
    return (double) val <= (double) target ? this.E.min(val + amount, target) : this.E.max(val - amount, target);
  }
  ...
  {
    return this.E.mget(this.room.X * 16 /*0x10*/ + x, this.room.Y * 16 /*0x10*/ + y);
  }
  ...
  public class player : Classic.ClassicObject
  {
    ...
    public override void update()
    {
      ...
      else
      {
        ...
        int num4 = 5;
        float num5 = (float) num4 * 0.707106769f;
        if (this.djump > 0 & flag3)
        {
          ...
          if ((double) this.spd.Y != 0.0)
            this.dash_accel.X *= 0.707106769f;
          if ((double) this.spd.X != 0.0)
            this.dash_accel.Y *= 0.707106769f;
        }
  ...
  public class fake_wall : Classic.ClassicObject
  {
    public override void update()
    {
      ...
      this.hitbox = new Rectangle(0, 0, 16 /*0x10*/, 16 /*0x10*/);
    }

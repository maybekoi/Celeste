...
public class GlassBlockBg : Entity
{
  ...
  public GlassBlockBg()
  {
    ...
    this.Add((Component) new DisplacementRenderHook(new Action(this.OnDisplacementRender)));
    this.Depth = -9990;
    ...
    for (int index = 0; index < this.rays.Length; ++index)
    {
      ...
      this.rays[index].Length = (float) Calc.Random.Choose<int>(48 /*0x30*/, 96 /*0x60*/, 128 /*0x80*/);
  ...
  private void BeforeRender()
  {
    ...
    if (this.starsTarget == null)
      this.starsTarget = VirtualContent.CreateRenderTarget("glass-block-surfaces", 320, 180);
    ...
    for (int index = 0; index < this.rays.Length; ++index)
    {
      ...
      else if ((double) position.X > (double) (num1 - 64 /*0x40*/))
        this.DrawRay(position - new Vector2((float) num1, 0.0f), ref vertex, ref this.rays[index]);
      if ((double) position.Y < 64.0)
        this.DrawRay(position + new Vector2(0.0f, (float) num2), ref vertex, ref this.rays[index]);
      else if ((double) position.Y > (double) (num2 - 64 /*0x40*/))
        this.DrawRay(position - new Vector2(0.0f, (float) num2), ref vertex, ref this.rays[index]);
    }
    if (this.beamsTarget == null)
      this.beamsTarget = VirtualContent.CreateRenderTarget("glass-block-beams", 320, 180);
    Engine.Graphics.GraphicsDevice.SetRenderTarget((RenderTarget2D) this.beamsTarget);
    Engine.Graphics.GraphicsDevice.Clear(Color.Transparent);
    GFX.DrawVertices<VertexPositionColor>(Matrix.Identity, this.verts, vertex);
  }

  private void OnDisplacementRender()
  {
    foreach (Entity entity in this.Scene.Tracker.GetEntities<GlassBlock>())

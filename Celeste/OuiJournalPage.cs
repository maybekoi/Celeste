...
public abstract class OuiJournalPage
{
  ...
  public class Table
  {
    ...
    {
      get => this.rows.Count <= 0 ? (OuiJournalPage.Row) null : this.rows[0];
    }
  ...
  public class TextCell : OuiJournalPage.Cell
  {
    ...
    {
      return this.forceWidth ? this.width : Math.Max(this.width, ActiveFont.Measure(this.text).X * this.scale);
    }

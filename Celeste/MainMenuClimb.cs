...
namespace Celeste
{
    ...
    public class MainMenuClimb : MenuButton
    {
      ...
      {
        this.label = Dialog.Clean("menu_begin");
        this.icon = GFX.Gui["menu/start"];
        this.labelScale = 1f;
        float num = ActiveFont.Measure(this.label).X * 1.5f;
        if ((double) num > 256.0)
          this.labelScale = 256f / num;
        this.Add((Component) (this.bounceWiggler = Wiggler.Create(0.25f, 4f)));
        this.Add((Component) (this.rotateWiggler = Wiggler.Create(0.3f, 6f)));
        this.Add((Component) (this.bigBounceWiggler = Wiggler.Create(0.4f, 2f)));
      }
      ...
      public override void Render()
      {
        ...
        ActiveFont.DrawOutline(this.label, this.Position + vector2_1 + new Vector2(0.0f, (float) (48 /*0x30*/ + this.icon.Height)), new Vector2(0.5f, 0.5f), Vector2.One * 1.5f * this.labelScale, this.SelectionColor, 2f, Color.Black);
      }

      public override float ButtonHeight
      {
        get => (float) ((double) this.icon.Height + (double) ActiveFont.LineHeight + 48.0);
      }
    }
}

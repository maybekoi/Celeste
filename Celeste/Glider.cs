...
public class Glider : Actor
{
    ...
    if (!this.destroyed)
    {
      ...
      else if (!this.bubble)
      {
        ...
        else
        {
          ...
          bounds = this.level.Bounds;
          double num = (double) (bounds.Bottom + 16 /*0x10*/);
          if (top3 > num)
          {
            this.RemoveSelf();
            return;
  ...
  private void PlayOpen()
  {
    ...
    this.level.Particles.Emit(Glider.P_Expand, 16 /*0x10*/, this.Center + (Vector2.UnitY * -12f).Rotate(this.sprite.Rotation), new Vector2(8f, 3f), this.sprite.Rotation - 1.57079637f);
    if (!this.Hold.IsHeld)
      return;
  ...
  {
    return new Vector2((float) (num - 12), (float) ((int) Math.Round(Math.Sin((double) this.Scene.TimeActive + (double) num * 0.20000000298023224) * 1.7999999523162842) - 5));
  }

  private Color PlatformColor(int num)
  {
    return num <= 1 || num >= 22 ? Color.White * 0.4f : Color.White * 0.8f;
  }

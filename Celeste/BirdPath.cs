...
public class BirdPath : Entity
{
  ...
  private IEnumerator Routine()
  {
    ...
    for (int i = 0; i <= birdPath.nodes.Length - 1; i += 2)
    {
      ...
      float duration = curve.GetLengthParametric(32 /*0x20*/) / birdPath.maxspeed;
  ...
  public override void DebugRender(Camera camera)
  {
    ...
    for (int index = 0; index < this.nodes.Length - 1; index += 2)
    {
      ...
      new SimpleCurve(begin, node, this.nodes[index]).Render(Color.Red * 0.25f, 32 /*0x20*/);
      begin = node;
    }
    ...
    Draw.Circle(this.target, 3f, Color.Yellow, 16 /*0x10*/);
  }
}

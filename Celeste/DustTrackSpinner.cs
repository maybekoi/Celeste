...
public class DustTrackSpinner : TrackSpinner
{
  ...
  private void Establish()
  {
    ...
    for (int index = 8; (double) index < (double) num & flag; index += 8)
    ...
    double startAngle = (double) this.outwards.Angle();
    double endAngle = this.Up ? (double) this.Angle + 3.1415927410125732 : (double) this.Angle;
    Vector2 vector;
  ...
  public override void OnTrackEnd()
  {
    ...
    {
      this.dusty.EyeTargetDirection = Calc.AngleToVector(Calc.AngleLerp(this.outwards.Angle(), this.Up ? this.Angle + 3.14159274f : this.Angle, 0.3f), 1f);
    }
    else
    {
      this.dusty.EyeTargetDirection = Calc.AngleToVector(this.Up ? this.Angle + 3.14159274f : this.Angle, 1f);

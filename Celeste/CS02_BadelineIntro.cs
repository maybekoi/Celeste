...
namespace Celeste
{
    ...
    public class CS02_BadelineIntro : CutsceneEntity
    {
      ...
      public override void Update()
      {
        ...
        Distort.Anxiety = this.anxietyFade * Math.Max(0.0f, (float) (0.0 + (double) this.anxietyJitter + (double) this.anxietySine.Value * 0.30000001192092896));
      }
      ...
      private IEnumerator TurnAround()
      {
        ...
        cs02BadelineIntro.Add((Component) new Coroutine(CutsceneEntity.CameraTo(new Vector2((float) cs02BadelineIntro.Level.Bounds.X, cs02BadelineIntro.Level.Camera.Y), 0.5f)));
      ...
      private IEnumerator RevealBadeline()
      {
        ...
        cs02BadelineIntro.Level.Displacement.AddBurst(cs02BadelineIntro.badeline.Position + new Vector2(0.0f, -4f), 0.8f, 8f, 48f, 0.5f);
      ...
      {
        yield return (object) 0.2f;
        this.badeline.Sprite.Play("laugh", true);
        yield return (object) null;
      }

      private IEnumerator StopLaughing()
      {
        this.badeline.Sprite.Play("fallSlow", true);
        yield return (object) null;
      }
      ...
      public override void OnEnd(Level level)
      {
        ...
        this.badeline.Sprite.Play("fallSlow");
        this.badeline.Hovering = false;
        this.badeline.Add((Component) new Coroutine(this.badeline.StartChasingRoutine(level)));
